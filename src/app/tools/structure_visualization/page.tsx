...
const StructureVisPage = () => {
  ...
  const [energyRowName, setEnergyRowName] = useState<string>("");
  const [relativeEnergyRowName, setRelativeEnergyRowName] = useState<string>("");
  ...
  const changeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(event.target.files[0], {
      ...
      complete: (results: { data: any[]; }) => {
        ...
        setEnergyRowName(rowsArray[0].find((row: string) => row.includes('torch_energy model')));
        setRelativeEnergyRowName(rowsArray[0].find((row: string) => row === 'torch_energy_relative_kJ/mol'));
  ...
  const plotMolecule = (moleculeId: number) => {
    ...
    return (
      <>
        ...
        />
        <div>
          Torch Energy: {molecule[energyRowName]} eV <br />
          Relative Energy: {molecule[relativeEnergyRowName]} kJ/mol
        </div>
      </>
    );
  };
    ...
    <div className="w-full text-left">
      ...
      <div className="w-full p-10">
        ...
        <div className="max-w-[1240px] mx-auto">
          ...
          <div className="max-w-[1240px] mx-auto">
            <p className="text-ml tracking widget uppercase text-[#5651e5] pt-4">
              How to use
            </p>
            ...
            <ul className="text-ml">
              <li><Link href="/example/C[C@@]12CCC3=C([C@@]14[C@H](C[C@@]2(C4=O)[C@@H]5C[C@H]5C(=O)O)O)C(=O)C(=C6[C@@]7([C@H]3O)CC(=O)[C@@H](C6(C)C)O7)OC.csv" className="text-blue-600 hover:cursor-pointer hover:border-b">Solanoceclepin A</Link></li>
            </ul>
          </div>
        </div>
